...
/*
  ...
  - When entry is removed, its key and data are cleared, but its hash value remains the
    same.
  - In other words, voided slots have empty key but non-zero hash value, and free slots
  ...
  - Empty table is represented by magic EMPTY table value.  This is compile-time constant
    data and will never vchange.
*/

#define EMPTY     (const_cast<Entry*>(reinterpret_cast<const Entry*>(__variantmap__empty__+3)))
...
void FXVariantMap::remove(const FXchar* ky){
  register FXuval p,b,h,x;
  if(__unlikely(!ky || !*ky)){ throw FXRangeException("FXVariantMap::const: null or empty key\n"); }
  p=b=h=FXString::hash(ky);
  FXASSERT(h);
